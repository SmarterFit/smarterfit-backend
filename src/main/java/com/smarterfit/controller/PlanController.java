...
public class PlanController {
   ...
   @PostMapping()
   public ResponseEntity<PlanResponseDTO> createPlan(@RequestBody @Valid PlanDTO planDTO) {
      PlanResponseDTO responseDTO = planService.createPlan(planDTO);
   ...
   @GetMapping("/search")
   public ResponseEntity<Page<PlanResponseDTO>> searchPlans(@ModelAttribute SearchDTO searchDTO, Pageable pageable) {
      return ResponseEntity.ok(planService.searchPlans(searchDTO, pageable));
   }

   @PutMapping("/{id}")
   public ResponseEntity<PlanResponseDTO> updatePlan(@PathVariable UUID id,
         @RequestBody @Valid PlanDTO planRequestDTO) {
