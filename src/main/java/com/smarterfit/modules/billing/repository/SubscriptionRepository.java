...
public interface SubscriptionRepository
        ...
        @Query("""
                        ...
                        WHERE su.user.id = :participantId
                           ...
                           AND cg.id = :classGroupId
                        """)
        List<Subscription> findAvailableSubscriptionsByClassGroupAndParticipant(
                        @Param("classGroupId") UUID classGroupId,
                        @Param("participantId") UUID participantId);

        // Renewals within window
        Long countByRenewedInBetween(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);

        // Creations within window
        Long countByCreatedAtBetween(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);

        // Cancellations within window
        Long countByStatusAndEndedInBetween(SubscriptionStatus status, LocalDateTime from, LocalDateTime to);

        // Pendings within window
